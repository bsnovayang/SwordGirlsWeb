...
產生 Episode 3 卡片的 JS 片段。

用法： python tools/gen_ep3.py > tools/ep3_cards.txt
"""
...
PAGES = [('77', 'vita', 'follower'), ('83', 'academy', 'follower'),
         ('89', 'crux', 'follower'), ('95', 'darklore', 'follower'),
         ('107', 'vita', 'spell'), ('115', 'academy', 'spell'),
         ('121', 'crux', 'spell'), ('127', 'darklore', 'spell')]
CHAR_PAGE = '55'
...
def main():
    ...
    for c in fd:
        ...
        if n not in BY or (c.get('title') == n and BY[n].get('title') != n):
            BY[n] = c
    mp = json.load(open(os.path.join(D, 'map_ep3.json'), encoding='utf-8'))

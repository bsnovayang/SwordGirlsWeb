...
產生 Episode EX1 卡片的 JS 片段。

用法： python tools/gen_ex1.py > tools/ex1_cards.txt
"""
import json, os, re, sys

D = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, D)
from at_ep import collect, align

PAGES = [('79', 'vita', 'follower'), ('85', 'academy', 'follower'),
         ('91', 'crux', 'follower'), ('97', 'darklore', 'follower'),
         ('111', 'vita', 'spell'), ('117', 'academy', 'spell'),
         ('123', 'crux', 'spell'), ('129', 'darklore', 'spell')]
...
def main():
    ...
    for c in fd:
        ...
        if n not in BY or (c.get('title') == n and BY[n].get('title') != n):
            BY[n] = c
    mp = json.load(open(os.path.join(D, 'map_ex1.json'), encoding='utf-8'))

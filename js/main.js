...
(function () {
  ...
  var TIPS = [
    ...
    '新手牌組的強弱原作就有差：南十字最好上手，暗黑族的初期牌效果比較微妙。實測勝率見規則頁。',
    '副本的難度來自 BOSS 的能力與生命（40~50），不是 AI 多聰明 —— 出牌策略是玩家的優勢。',
    'Episode 1 的卡片要用竹林鄉的「竹」與邊境遺跡的「遺跡碎片」才做得出來。'
  ];
  ...
  function rules() {
    $('rulesBody').innerHTML =
      ...
      '<h3>新手牌組的強弱</h3>' +
      '<p>四副新手牌組本來就不是五五波。繁中 wiki 的 FAQ 直接寫過：' +
      '<b>南十字</b>（強化為主）最好上手，其次是<b>私立</b>（弱化、搗亂）、' +
      '<b>公立</b>（強化弱化搗亂皆有），最後是<b>暗黑族</b>（初期牌效果微妙）。</p>' +
      '<p class="hint left">本作實測 AI 互打 1440 場的勝率：私立 66%／公立 57%／' +
      '南十字 50%／暗黑 28%。<b>暗黑族墊底跟原作說法一致</b>，' +
      '但南十字沒有排到前面 —— wiki 講的是「好上手」，不等於對 AI 的勝率。' +
      '這是刻意保留的，不是把數值調成平衡 —— 暗黑族要靠 Episode 1 之後的卡才會起來。</p>' +
      '<h3>牌組</h3><ul>' +
      ...
      '<h3>副本的戰鬥評價</h3>' +
      '<p>副本每贏一場都會結算<b>戰鬥評價</b>，分數決定拿到多少素材 ——' +
      '不是每場都固定給那幾個。加分項目：</p><ul>' +
      '<li><b>勝利</b> +100</li>' +
      '<li><b>生命殘量</b> 最多 +100（留得越多越高）</li>' +
      '<li><b>速攻</b> 最多 +100（第 1 回合結束滿分，每多一回合 −12，第 9 回合起歸零）</li>' +
      '<li><b>殲滅</b> 每擊破一張敵方隨從 +12</li>' +
      '<li><b>完封</b> 生命全滿再 +60</li>' +
      '</ul><p>再乘上倍率：<b>難度</b>（Normal ×1.35／Hard ×1.7）、' +
      '<b>樓層深度</b>（第 1 層 ×1.0 → 最深層 ×1.5）、<b>BOSS 戰</b> ×1.5。' +
      ...
      '<h3>副本的樓層</h3><ul>' +
      '<li>贏了往上一層、輸了往下一層；輸給 BOSS 退回第 1 層</li>' +
      '<li><b>打過的樓層可以點選重打</b> —— 練功、刷素材用</li>' +
      '<li>重打舊樓層照樣拿素材與卡包點數，但<b>贏了不會推進進度、輸了也不會退層</b></li>' +
      '<li>要往上、要通關，就得打目前所在的那一層</li>' +
      '</ul>' +

      '<h3>卡包與分解</h3><ul>' +
      '<li>副本每贏一關 <b>+1 點</b>，打贏 BOSS <b>+2 點</b></li>' +
      '<li>一包 <b>1 點</b>，開 3 張；<b>十連抽 10 點</b>，保證至少一張稀有以上</li>' +
      '<li><b>卡包分「陣營 × 章節」</b>，可以只抽你要的那一類 ——' +
      '全部混在一起的話，指定一張雙稀有要 133 包，選定陣營與章節後只要 17 包</li>' +
      '<li>連續 30 張沒開到稀有以上會<b>保底</b></li>' +
      '<li>稀有度機率：普通 66%／罕見 23%／稀有 9%／雙稀有 2%</li>' +
      '<li>抽到<b>已達牌組上限</b>的卡會直接折成素材，不會白抽</li>' +
      '<li><b>Episode 2 的卡包</b>要先通關一座 Normal 難度的副本才開放</li>' +
      '<li>副本通關 10 次的獎勵角色卡<b>不在卡池裡</b>，只能靠通關拿</li>' +
      '<li><b>分解</b>：超過牌組上限的多餘張數可以拆成素材（回收配方的 1/4）</li>' +
      '</ul>' +
      '<p class="hint left">合成是「補齊你缺的那幾張」，卡包是「立刻開出東西」——' +
      '抽到重複的也不浪費，會自動或手動變回素材。</p>';
  }
